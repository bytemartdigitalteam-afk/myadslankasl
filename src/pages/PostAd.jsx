...
const PostAd = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess('Ad posted successfully! It will be visible after admin approval.');
      setFormData({ title: '', description: '', price: '', category: 'Vehicles', location: '' });
      setImages([]);
      setPreviews([]);

      setTimeout(() => navigate('/dashboard'), 2000);
    } catch (err) {
      setError(err.response?.data?.message || err.message || 'Failed to post ad. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="bg-white dark:bg-dark-800 p-8 rounded-lg shadow-md border border-gray-200 dark:border-dark-700">
        <h1 className="text-3xl font-bold mb-6 text-gray-900 dark:text-white">Post Your Ad</h1>
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Ad Title</label>
            <input
              type="text"
              name="title"
              value={title}
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <input
                ...
                className="w-full bg-gray-50 dark:bg-dark-900 border border-gray-300 dark:border-dark-700 rounded py-2 px-3 text-gray-900 dark:text-gray-100 focus:outline-none focus:border-brand-magenta"
              />
              {paymentSlip && <span className="text-sm text-green-600 font-medium whitespace-nowrap">File Selected</span>}
            </div>
            <p className="text-xs text-gray-500 mt-1">Upload proof of payment to expedite approval.</p>
          </div>
